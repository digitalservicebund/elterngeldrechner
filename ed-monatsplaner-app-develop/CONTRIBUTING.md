...
Hello and welcome to the ED/EO/EGR Monatsplaner App. This repository will provide the API for this application.
We want to distribute the following functionality in this package:

1. Monatsplaner API
2. Monatsplaner jQuery Plugin
3. Monatsplaner React Component
...
#### Scss
We want to use `#{$nsp}` as prefix for our BEM organized css classes.\
We want to use `$ed-`-prefixed variable names.\
We want to use  this pattern for variable naming: `[prefix]-[component-block][__component-element|--modfier][--state]--[shortend-css-attribute]--(viewport)`\
We want to use the `Mobile-First`-Concept to style our components.\

```scss
.#{$nsp}my-component {
  font-size: rem($ed-my-component--fs);
  // probably better: rem($ed-my-component--fs--sm)
  
  &--small {
    font-size: rem($ed-my-component--small--fs);
  }

  &__heading {
    font-size: rem($ed-my-component__heading--fs);
    
    &:hover {
        font-size: rem($ed-my-component__heading--hover--fs);
    }

    @include media-breakpoint-up(lg) {
      font-size: rem($ed-my-component__heading--fs--lg);
    }
  }
}
```
...
```bash
my-new-component
|- settings.scss: (*) will provide all scss variables
