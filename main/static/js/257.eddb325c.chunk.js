...
/* harmony export */   qo: () => (/* binding */ useWatch)
/* harmony export */ });
/* unused harmony exports Controller, Form, appendErrors, set, useFormState */
...
__webpack_require__.d(__webpack_exports__, {
  LV: () => (/* reexport */ holder_IMask),
  oc: () => (/* reexport */ IMaskInput)
});

// UNUSED EXPORTS: IMaskMixin, useIMask
  ...
  if (holder_IMask.Masked) {
    ...
    */
    const {
      mask = undefined,
      ...instanceOpts
    } = opts instanceof holder_IMask.Masked ? {
      mask: opts
    } : isObject(opts) && opts.mask instanceof holder_IMask.Masked ? opts : {};
    if (mask) {
    ...
    for (let i = 0; i < pattern.length; ++i) {
      if (this.blocks) {
        ...
        if (bName) {
          ...
          const maskedBlock = repeat != null ? new holder_IMask.RepeatBlock(blockOpts /* TODO */) : createMask(blockOpts);
          if (maskedBlock) {
...
const MASK_PROPS = {
  // common
  mask: prop_types.oneOfType([prop_types.array, prop_types.func, prop_types.string, prop_types.instanceOf(RegExp), prop_types.oneOf([Date, Number, holder_IMask.Masked]), prop_types.instanceOf(holder_IMask.Masked)]),
...
function IMaskMixin(ComposedComponent) {
  ...
  const MaskedComponent = (_Class = class MaskedComponent extends react.Component {
    ...
    initMask(maskOptions) {
      ...
      this.maskRef = holder_IMask(this.element, maskOptions).on('accept', this._onAccept.bind(this)).on('complete', this._onComplete.bind(this));
