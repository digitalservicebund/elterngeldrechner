...
export async function fetchTagManagerData(date: Date) {
  ...
  const url = `https://${process.env.EGR_METRICS_SYNC_MATOMO_DOMAIN}/index.php?module=API&format=JSON&idSite=86&period=day&date=${formattedDate},${formattedDate}&method=Events.getCategory&filter_limit=100&format_metrics=1&expanded=1&token_auth=${process.env.EGR_METRICS_SYNC_MATOMO_AUTHENTICATION_TOKEN}&force_api_session=1`;
