...
describe("Hinweis zum Bonus", () => {
  it("shows information for automatic selection of two Lebensmonate", () => {
    ...
    expect(
      screen.queryByText(
        /Sie müssen den Partnerschaftbonus für mindestens 2 Lebensmonate beantragen – deshalb wurde automatisch auch der folgende Monat ausgewählt. Sie können bis zu vier Lebensmonate Partnerschaftsbonus bekommen./,
      ),
    ).toBeVisible();
  });
  ...
  it("shows hint for automatic parallel selection if has multiple Elternteile", () => {
    ...
    expect(
      screen.queryByText(
        /Wenn Sie den Partnerschaftsbonus auswählen, wird er auch automatisch für den anderen Elternteil ausgewählt. Der Partnerschaftsbonus kann immer nur parallel genommen werden./,
      ),
    ).toBeVisible();
  });
