 ...
 * Generated at: 2026-07-15T10:14:33.285584
...
interface EventSchemas {
  angaben_gemeinsame_planung_gemacht: {
    planen_gemeinsam: string;
  };
  $autocapture: Record<string, any>;
  beispiel_geklickt: {
    identifier: string;
  };
  beispiel_uebernommen: {
    identifier: string;
  };
  beispiel_wurde_angeklickt: {
    identifier: string;
  };
  beispiel_wurde_uebernommen: {
    identifier: string;
  };
  browser_zurueck_button_geklickt: {
    route: string;
  };
  eingabe_validierungsfehler: {
    fehlermeldung: string;
    feld: string;
  };
  $exception: Record<string, any>;
  $identify: Record<string, any>;
  info_text_ausgeklappt: {
    frage: string;
  };
  monatsplaner_pruefbutton_geklickt: {
    bonustip?: boolean;
    gueltig: boolean;
    tips?: number;
  };
  navigationspunkt_geklickt: {
    abschnitt: string;
  };
  navigationspunkt_wurde_geklickt: {
    abschnitt: string;
  };
  ...
  taetigkeiten_angaben_abgeschlossen: {
    anteil_minijobs?: number;
    anteil_ohne_sozialversicherungen?: number;
  };
  zurueck_button_geklickt: {
    route: string;
  };
}
...
type HasRequiredProperties<K extends EventName> = {} extends EventSchemas[K]
  ? false
  : true;

// Helper to detect if T is exactly 'string' (not a literal)
type IsExactlyString<T> = string extends T
  ? T extends string
    ? true
    : false
  : false;
...
interface TypedPostHog extends Omit<OriginalPostHog, "capture"> {
  ...
  capture<T extends string>(
    event_name: IsExactlyString<T> extends true
      ? never
      : T extends EventName
        ? never
        : T,
