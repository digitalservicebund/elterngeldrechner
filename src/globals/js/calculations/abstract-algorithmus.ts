...
export function ersatzrate_eg(ek_vor_copy: number): number {
...
export function elterngeld_keine_et(ekVor: number): number {
...
export function elterngeld_et(ekVor: number, ekNach: number): number {
  let ersatzrate = 0;
...
export function elterngeldplus_et(ekVor: number, ekNach: number): number {
  let ersatzrate = 0;
  const ek_diff = Math.max(Math.min(ekVor, HOECHST_ET) - ekNach, 0);
